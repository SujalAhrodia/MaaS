...
zones={'zone1':['172.16.3.1','linux@1234'],'zone2':['172.16.3.2','linux@123']}
...
def attach_interface(vm_id,tenant_id,subnet,ip,zone_id,route_flag):
    try:
        zone_ssh=create_ssh_conn(zones[zone_id][0],'ece792',zones[zone_id][1])
        ...
        mac=execute_command(zone_ssh,'sudo virsh domiflist '+vm_id+' | grep '+vm_id+subnet+' | awk \'{print $5}\'')
        ...
        if route_flag:
            ...
            execute_command(vm_ssh, "ip route add default dev " + dev_name + "via " + gw_ip)
